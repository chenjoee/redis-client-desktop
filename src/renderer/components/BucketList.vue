<template>
    <el-menu>
        <el-submenu  :index="''+index" v-for="(item,index) in getRedisConnList">
            <template slot="title" >
                <i class="icon"></i>
                <span @click="openConn(item)" slot="title">{{item.name}}</span>
            </template>
            <el-menu-item :index="index + '-' + index2" v-for="(index2) in dbindex">
                <span @click="selectDB(item, index2)">db-{{index2}}</span>
            </el-menu-item>
        </el-submenu>
    </el-menu>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            openConn(client) {
                ...
                // client.dbinfo = [1,3,4,5,6]

            },
            selectDB(mutClient, dbindex) {
                console.log(mutClient.client, dbindex);
                let clinet = mutClient.client;
            },
